...
wait_for_serial_prompt() {
    ...
    wait=1
    while true; do
        if grep -q "$prompt" "$work_dir/serial.log" ; then
            break
        fi
        if ! systemctl is-active "$service_name"; then
            echo "Service no longer active"
        ...
        retry=$(( retry - 1 ))
        if [ $retry -le 0 ]; then
            echo "Timed out waiting for serial log to contain '$prompt'. Aborting!"
            return 1
        fi
        echo "-- last log"
...
get_nested_svc_name() {
    echo "nested-vm-$(systemd-escape "${SPREAD_JOB:-unknown}")"
}
...
start_nested_core_vm_unit(){
    ...
    # TODO figure out how to interact with the serial
    ...
    if [ "${ENABLE_SECURE_BOOT:-false}" = "true" ]; then
        VMF_CODE="${VMF_CODE}.ms"
    fi
    if [ "${ENABLE_OVMF_SNAKEOIL:-false}" = "true" ]; then
        VMF_VARS="${VMF_VARS}.snakeoil"
    fi
    ...
    # wait until we boot into the run mode
    if ! wait_for_serial_prompt "$SVC_NAME" "$WORK_DIR" "Command line: snapd_recovery_mode=run"; then
        return 1
    fi
}

nested_vm_screenshot() {
    local out="$1"
    vmcli --monitor "localhost:$MON_PORT" screenshot --output "$out.ppm"
    convert "$out.ppm" "$out"
    rm "$out.ppm"
}

nested_vm_send_input() {
    local what="$1"
    vmcli --monitor "localhost:$MON_PORT" send-input "$what"
}

nested_vm_send_raw_input() {
    local what="$1"
    vmcli --monitor "localhost:$MON_PORT" send-input --raw "$what"
}
